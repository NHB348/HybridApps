...
WL.ClientMessages = {
		  ...
		  "directUpdateNotificationMessage": "An update for the application is available (file size is {0} MB).",
		  "close": "Close",
		  ...
		  "error": "Error",
		  "directUpdateNotificationMessageKilobytes": "An update for the application is available (file size is {0} KB).",
		  ...
		  "applicationDenied": "Access to Server is Disabled",
		  "diagTime": "Time",
...
WLAuthorizationManager.LOCAL_STORAGE_PERSISTENCE_POLICY = 'com.worklight.oauth.persistence.policy';
WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID = 'com.worklight.oauth.clientid';
WLAuthorizationManager.LOCAL_STORAGE_CLIENT_SECRET = 'com.worklight.oauth.clientsecret';
WLAuthorizationManager.LOCAL_STORAGE_ACCESS_TOKEN = 'com.worklight.oauth.accesstoken';
WLAuthorizationManager.LOCAL_STORAGE_KEY_ID_TOKEN = 'com.worklight.oauth.idtoken';
...
WLAuthorizationManager.setAuthorizationPersistencePolicy = function(authorizationPersistencePolicy) {
	...
	if (authorizationPersistencePolicy !== WLAuthorizationManager.__getPersistencePolicy()) {
		...
		WLAuthorizationManager.__copyDataAndClear(copyToStorage, WLAuthorizationManager.LOCAL_STORAGE_ACCESS_TOKEN);
		WLAuthorizationManager.__copyDataAndClear(copyToStorage, WLAuthorizationManager.LOCAL_STORAGE_KEY_ID_TOKEN);
		
		localStorage.setItem(WLAuthorizationManager.LOCAL_STORAGE_PERSISTENCE_POLICY, authorizationPersistencePolicy);
	}
...
WLAuthorizationManager.__getPersistencePolicy = function() {
	var policy = localStorage.getItem(WLAuthorizationManager.LOCAL_STORAGE_PERSISTENCE_POLICY);
	
	if (policy === null) {
		localStorage.setItem(WLAuthorizationManager.LOCAL_STORAGE_PERSISTENCE_POLICY, WLAuthorizationManager.AuthorizationPersistencePolicy.ALWAYS);
...
WLAuthorizationManager.__copyDataAndClear = function(toStorage, key) {
	if (toStorage) {
		localStorage.setItem(key, WLAuthorizationManager.authenticationData[key]);
		delete WLAuthorizationManager.authenticationData[key];
	} else {
		WLAuthorizationManager.authenticationData[key] = localStorage.getItem(key);
		localStorage.removeItem(key);
	}
}
...
WLAuthorizationManager.getCachedAuthorizationHeader = function() {
	var dfd = WLJQ.Deferred();
	var accessToken = WLAuthorizationManager.__getAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_ACCESS_TOKEN) || null;
	var idToken = WLAuthorizationManager.__getAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_KEY_ID_TOKEN) || null;
...
WLAuthorizationManager.__getClientInstanceIdHeader = function() {
	...
	var savedClientId = localStorage.getItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID);
		...
		function(serverUrl) {
			...
			function onSuccess(response) {
				if (typeof(response.responseJSON) === 'undefined' || typeof(response.responseJSON.clientId) === 'undefined') {
					...
					localStorage.setItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID, response.responseJSON.clientId);
					localStorage.setItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_SECRET, response.responseJSON.clientSecret);
...
WLAuthorizationManager.__sendRequestToTokenEndpoint = function(serverUrl, grantCode) {
	...
	var requestOptions = {
			method : 'POST',
			parameters : {
				client_id : localStorage.getItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID),
	...
	function onSuccess(response) {
		WLAuthorizationManager.__setAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_ACCESS_TOKEN, response.responseJSON.access_token);
		WLAuthorizationManager.__setAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_KEY_ID_TOKEN, response.responseJSON.id_token);
	...
	var authContent = localStorage.getItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID) + ':' + 
						localStorage.getItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_SECRET);
...
WLAuthorizationManager.__getClientId = function() {
	var dfd = WLJQ.Deferred();
	var clientId = localStorage.getItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID);
...
WLAuthorizationManager.__deleteAuthData = function() {
	var dfd = WLJQ.Deferred();
	localStorage.removeItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_ID);
	localStorage.removeItem(WLAuthorizationManager.LOCAL_STORAGE_CLIENT_SECRET);
	WLAuthorizationManager.__removeAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_ACCESS_TOKEN);
	WLAuthorizationManager.__removeAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_KEY_ID_TOKEN);
	WLAuthorizationManager.__cachedWlSessionId = null;
	dfd.resolve();
	return dfd.promise();
}
...
WLAuthorizationManager.__parseIdTokenToJSON = function() {
	var dfd = WLJQ.Deferred();
	var idToken = WLAuthorizationManager.__getAuthDataItem(WLAuthorizationManager.LOCAL_STORAGE_KEY_ID_TOKEN);
